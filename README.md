...
This source tree is the canonical source for the external plugin repo used by the live pipeline (`github.com/Bande-a-Bonnot/tart-bk-plugin#v0.1.1`). BuildKite must load this as a non-vendored plugin so its `checkout` hook is available before repository checkout.
