...
if grep -E '(^|[[:space:]])(export[[:space:]]+)?(DOPPLER_TOKEN|FOUNDRY_GITHUB_APP_PRIVATE_KEY)=' "$ENV_FILE" "$CMD_FILE" || \
   grep -E 'BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY|super-secret-doppler' "$ENV_FILE" "$CMD_FILE"; then
